<img align="left" src="https://github.com/wednesday-solutions/go-template-consumer/blob/master/golang_template_github.svg" width="440" height="480" />
...
Using go-template-consumer requires having Go 1.7 or above. Once you download go-template-consumer (either using Git or go get) you need to configure the following:

1. Run the `pre-commit-setup.sh` script to install the pre-commit hooks locally.
...
- **`make local`**  
  ...
  This starts the containers in `local` stage, bind the `current directory` to `/go/src/server` inside the `go-template-consumer_server_1` container and then starts the `terminal` inside `go-template-consumer_server_1`. Once the development is over, `exit` the terminal and call `make tear env=local` to stop all the containers
...
```txt
go-template-consumer/
└──.github/
│  └──workflow/go-template-consumer-ci.yml   # this file contains the config of github action
└──cmd/
│  └──seeder/
